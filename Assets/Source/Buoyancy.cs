...
//	public Ocean ocean;
    ...
    public Collider colliderForBuoyancy;
	...
	private void Start()
	{
        ...
        Collider collider = colliderForBuoyancy;
        if (collider == null)
        {
		...
		cachedRigidBody.centerOfMass = new Vector3(0, -bounds.extents.y * 0f, 0) + transform.InverseTransformPoint(bounds.center);

		voxels = SliceIntoVoxels(collider);

		// Restore original rotation and position
		transform.rotation = originalRotation;
		transform.position = originalPosition;

		float volume = cachedRigidBody.mass / density;
		...
		Debug.Log(string.Format("[Buoyancy.cs] Name=\"{0}\" volume={1:0.0}, mass={2:0.0}, density={3:0.0}", name, volume, cachedRigidBody.mass, density));
	}

	/// <summary>
	/// Slices the object into number of voxels represented by their center points.
	/// <param name="collider">The collider to use for buoyancy.</param>
	/// <returns>List of voxels represented by their center points.</returns>
	/// </summary>
	private List<Vector3> SliceIntoVoxels( Collider collider)
	{
...
//		return ocean == null ? 0.0f : ocean.GetWaterHeightAtLocation(x, z);
	...
	private void FixedUpdate()
	{
		...
		foreach (var point in voxels)
		{
			...
			if (wp.y - voxelHalfHeight < waterLevel)
			{
				...
				var velocity = cachedRigidBody.GetPointVelocity(wp);
				var localDampingForce = -velocity * DAMPFER * cachedRigidBody.mass;
				var force = localDampingForce + Mathf.Sqrt(k) * localArchimedesForce;
				cachedRigidBody.AddForceAtPosition(force, wp);
	...
	private void OnDrawGizmos()
	{
		...
		foreach (var force in forces)
		{
			...
			Gizmos.DrawLine(force[0], force[0] + force[1] / cachedRigidBody.mass);
		}
	}
}